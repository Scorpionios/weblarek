...
##### Интерфейс Product
Сущность, описывающая данные товара и необходимая для классов Catalog и Basket.

Он состоит из данных:
    `id: string;` - уникальный индификационный номер товара.
    `description: string;` - описание товара.
    `image: string;` - изображение товара.
    `title: string;` - название товара.
    `category: string;` - категория, к которой относиться товар.
    `price: number | null;` - цена товара.

##### Интерфейс IBuyer
Сущность, описывающая данные покупателя и необходимая для класса Customer.

Он состоит из данных:
    `payment: payment;` - способ оплаты.
    `email: string;` - почта покупателя.
    `phone: string;` - номер телефона покупателя.
    `address: string;` - адресс доставки товара.

##### Тип Payment
Тип payment может принимать только значения "card" | "cash" | "".

##### Интерфейс ResponseProducts
Для получения объекта с данными о товарах с сервера.
...
Для получения объекта о покупке с сервера.

Он состоит из:
    `total: number` - обозначает общую стоимость
    `id: string` - id покупки

##### Интерфейс Order
Для отправки объекта о покупке на сервер

Он состоит из данных из интерфейса IBuyer и
`total: number` - количество купленных товаров
`items: string[]` - массив с id товарами

##### Тип TBuyerErrors
Нужен для получения объекта при валидации данных пользователя
`Partial<Record<keyof IBuyer, string>>`

##### Тип ProductData
Нужен для создания абстрактного класса в слое представления для товаров
`Pick<IProduct, 'title' | 'price'>`

##### Интерфейс ICardActions
Нужен для получения функции при создании каждой карточки товара
    `onClick?: () => void;`

##### Интерфейс IFormState
Нужен для создания абстрактного класса в слое представления для формы
    `error: string;`
    `valid: boolean;`

##### Тип ProductMidtermData
Нужен для создания промежуточного класса для товаров в каталоге
`Pick<IProduct, 'category' | 'image'> & ProductData`

##### Тип TOrderFormChange
Нужен для изменнения данных в форме
`| { field: 'payment'; value: Payment }`
`| { field: 'address'; value: string }`

##### Тип TContactsFormChange
Нужен для изменнения данных в форме
...
Поля класса:
`items: IProduct[] = []` - хранит массив с товарами.
`card: IProduct` - хранит товар, выбранный для подробного отображения.
...
`setFindId (enteredId: string): IProduct | undefined` - получение одного товара по его id.
`setCard (enteredId: string): void` - сохранение товара для подробного отображения.
`getCard (): IProduct | undefined` - получение товара для подробного отображения.
...
`addItem (item: IProduct, products: IProduct[]): void` - добавление товара, который был получен в параметре и есть в каталоге, в массив корзины.
`deletItem (enteredId: string): void` - удаление товара, полученного в параметре из массива корзины.
...
Поля класса:
`user: IBuyer` - хранит объект с данными покупателя.

Методы класса:
`setuser (updates: Partial<IBuyer>): void` - сохранение данных о покупателе.
`getuser (): IBuyer` - получение всех данных покупателя.
`deleteUser (): void` - очистка данных покупателя.
`validation (): Partial<IBuyer>` - валидация данных.
...
Поля класса:
`api: IApi` - хранит класс с методами.
...
#### Класс Product
Родительский класс для классов ProductBasket, ProductMidterm
...
#### Класс ProductMidterm
Родительский класс для ProductCatalog и ProductPreview

Конструктор:
`constructor(container: HTMLElement)` - в конструктор передаётся HTML разметка с товаром и функция, которая запускается при нажатии на кнопку
...
Конструктор:
`constructor(protected events: IEvents, container: HTMLElement)` - в конструктор помещается HTML элемент с формой

Поля класса:
`protected errorForm: HTMLElement` - хранит HTML разметку строки для отображения ошибок
`protected submitButton: HTMLButtonElement` - хранит HTML разметку кнопки для отправки формы
...
Поля класса:
`protected emailInput: HTMLInputElement` - хранит HTML разметку инпута для заполнения формы
`protected phoneInput: HTMLInputElement` - хранит HTML разметку инпута для заполнения формы
...
Конструктор:
`constructor(container: HTMLElement)` - в конструктор передаётся HTML разметка формы и events для отправки emit
